          ...
          class="handle handlePrev active" tabindex="0" role="button" aria-label="Ver títulos anteriores"><b class="indicator-icon icon-leftCaret"></b></span> }
            ...
            <div className="sliderMask showPeek">
              ...
              className="sliderContent row-with-x-columns">
                { props.animes && props.animes.map((anime, index) => (
                   index < 12 &&
                   <ItemSlide
            ...
            { props.animes && document.querySelector('.title-card') && ( positionSlider + 1 ) * (window.window.innerWidth / document.querySelector('.title-card').offsetWidth) < props.animes.length &&
            <span 
            onClick={() => {
              props.animePreview({})
              if(document.querySelector('.title-card')){
                if((( positionSlider + 1 ) * window.window.innerWidth / document.querySelector('.title-card').offsetWidth) < props.animes.length ){
                  var old = positionSlider
                  old++
                  setPositionSlider(old)
                } else{
            ...
            onMouseEnter={
              async e => {
                if(true){
                  ...
                  link = link.replace(`Dublado`, ``)
                  link = link.replace(`Legenadado`, ``)
...
export default CarrouselSearch;